...
| -------- | ------------ | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `GET`    | `/health`    | Liveness probe. Returns `{status, version, mode, worker, worker_ipc}` without touching the worker; `?deep=1` also fetches the worker's health body and merges its `runtime` object into the response (`runtime.playback_ready` is true when FPS decrypt is available).                                                                                                             |
| `GET`    | `/me`        | `{version, runtime, auth}` — same runtime flags as `GET /health?deep=1`.                                                                                                                                                                                                                                                                                                           |
